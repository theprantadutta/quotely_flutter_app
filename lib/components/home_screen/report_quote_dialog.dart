...
import '../../dtos/quote_dto.dart'; // Make sure this path is correct
...
class _ReportQuoteDialogState extends State<ReportQuoteDialog> {
  final TextEditingController _issueController = TextEditingController();
  final TextEditingController _correctInfoController = TextEditingController();
  final TextEditingController _contactEmailController = TextEditingController();

  String? _selectedReason;
  final List<String> _reasons = [
    'Quote Misattributed (Wrong Author)',
    'Quote Content Incorrect',
  ...
  void dispose() {
    ...
    _correctInfoController.dispose();
  ...
  Future<void> _submitReport() async {
    ...
    final String reportedQuoteId = widget.quote.id;
    final String reportedQuoteContent = widget.quote.content;
    final String reportedQuoteAuthor = widget.quote.author;
    final String selectedReason = _selectedReason ?? 'No reason selected';
    final String issueDetails = _issueController.text.trim();
    final String correctInfo = _correctInfoController.text.trim();
...
Reported Author: $reportedQuoteAuthor
Category: ${widget.quote.tags.join(', ')}

Reason: $selectedReason
Issue Details: ${issueDetails.isNotEmpty ? issueDetails : 'N/A'}
Suggested Correction (Quote/Author): ${correctInfo.isNotEmpty ? correctInfo : 'N/A'}
      ...
      if (await launchUrl(emailLaunchUri)) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Quote reported successfully! Thank you.'),
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    // Quote info section
                    Text(
                      'You are reporting the quote by ${widget.quote.author}:',
                    ...
                    if (_selectedReason == 'Other (Please specify below)') ...[
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintText:
                              'e.g., This quote is outdated, context is missing, etc.',
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(12),
                          ),
                    ...
                    // Correction field
                    Text(
                      'Suggested correction / correct author (optional):',
                      style: theme.textTheme.titleMedium?.copyWith(
                        fontWeight: FontWeight.w600,
                      ),
                    ),
                    const SizedBox(height: 12),
                    TextField(
                      controller: _correctInfoController,
                      decoration: InputDecoration(
                        labelText: 'Correct Quote / Author Info',
                        hintText:
                            'Provide the accurate quote or author if you know it.',
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(12),
                        ),
                        alignLabelWithHint: true,
